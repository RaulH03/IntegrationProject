...
def objective_global(p, u_data, x_meas, dt, Kt):
    x_sim = simulate_n_step(p, u_data, x_meas, dt, Kt, n_steps=50, delay_steps=1)
    if np.isinf(x_sim[0,0]): return 1e6
    
    mse_th1 = np.mean((x_sim[0, :] - x_meas[0, :])**2)
    mse_th2 = np.mean((x_sim[1, :] - x_meas[1, :])**2)
    mse_dq1 = np.mean((x_sim[2, :] - x_meas[2, :])**2) * 0
    mse_dq2 = np.mean((x_sim[3, :] - x_meas[3, :])**2) * 0
    
    # Weight positions heavily, keep velocities to constrain explosions
    return (mse_th1 * 1.0) + (mse_th2 * 3.0) + (mse_dq1 * 0.1) + (mse_dq2 * 0.1)

if __name__ == "__main__":
    KNOWN_G, KNOWN_KT = 9.81, 2.73
    
    # 1. Tight constraints to force physical compliance
    phys_min = {'m1': 0.05, 'm2': 0.01, 'I1': 0.0001, 'I2': 0.0001, 'l1': 0.1, 'l2': 0.1, 'b1': 0.0, 'b2': 0.0, 'dry1': 0.0, 'dry2': 0.0, 'g': KNOWN_G}
    phys_max = {'m1': 0.5,  'm2': 0.2,  'I1': 0.1,   'I2': 0.05,   'l1': 0.1, 'l2': 0.1, 'b1': 5.0, 'b2': 0.5, 'dry1': 2.0, 'dry2': 0.2, 'g': KNOWN_G}
    phys_guess = {'m1': 0.11, 'm2': 0.03, 'I1': 0.004, 'I2': 0.001, 'l1': 0.1, 'l2': 0.1, 'b1': 0.85, 'b2': 0.001, 'dry1': 0.05, 'dry2': 0.05, 'g': KNOWN_G}
    ...
    res = differential_evolution(
        objective_global, bounds=de_bounds, args=(u_data, x_meas, dt, KNOWN_KT),
        strategy='best1bin', popsize=15, disp=True, tol=1e-2,
        workers=-1, updating='deferred'
    )
    
    print("\n--- COPY THIS LIST INTO validate.py ---")
    print(list(np.round(res.x, 5)))