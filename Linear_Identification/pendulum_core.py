...
def _auto_derive_math():
    ...
    l1, l2, I1, I2, m1, m2, b1, b2, g, u, Kt_sym = sm.symbols(
        "l1 l2 I1 I2 m1 m2 b1 b2 g u Kt", real=True, positive=True
    )
    ...
    Po1.set_pos(O, r_o1)
    Po2.set_pos(O, r_o2)
    Po1.set_vel(N, r_o1.dt(N))
    Po2.set_vel(N, r_o2.dt(N))
    ...
    forces = [
        (A1, (-Kt_sym * u - b1 * th1_d + b2 * (th2_d - th1_d)) * N.z),
        (A2, -b2 * (th2_d - th1_d) * N.z),
    ]
...
def get_lumped_bounds(p_guess, p_min, p_max):
    ...
    P_guess = [float(f(*[p_guess[k] for k in keys])) for f in _P_funcs] + [
        p_guess["dry1"],
        p_guess["dry2"],
    ]
    P_lower = [float(f(*[p_min[k] for k in keys])) for f in _P_funcs] + [
        p_min["dry1"],
        p_min["dry2"],
    ]
    P_upper = [float(f(*[p_max[k] for k in keys])) for f in _P_funcs] + [
        p_max["dry1"],
        p_max["dry2"],
    ]
...
def augment_input(u_raw, dq1_meas, p, Kt, delay_steps=1):
    ...
    fric_val = (
        p[7] * ((1 + vel_sign) / 2.0) + p[8] * ((1 - vel_sign) / 2.0)
    ) * vel_sign
...
def simulate_n_step(p, u_data, x_meas, dt, Kt, n_steps=100, delay_steps=1):
    ...
    if A is None:
        return np.inf * np.ones((4, len(u_data)))

    Ad, Bd, _, _, _ = cont2discrete(
        (A, B, np.eye(4), np.zeros((4, 1))), dt, method="zoh"
    )
